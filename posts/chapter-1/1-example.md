...
이전 장([Introduction](./posts/0-intro.md)에서 우리는 일반화(generalization) 이 패턴인식의 주요 목적이라고 했다. 좋은 일반화란 얼만큼 진리에 가까운 표현력을 보이는가로 측정할 수 있다. 즉, 여기서는 새로운 데이터가 들어왔을 때, 얼만큼 $$\sin(2\pi x)$$ 에 접근한 가? 를 보면 된다. 그렇다면 이를 어떻게 측정할 것인가?
